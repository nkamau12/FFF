...
function setkeys(index)
    ind = index
    if(myData.BonusRescue.key[ind][1] ~= nil)then
      keyx = myData.BonusRescue.key[ind][1]
      if(keyx == 0) then
        key[ind][1] = 0
      elseif(keyx == 1) then
        key[ind][1] = 100
    ...
    if(myData.BonusRescue.key[ind][1] ~= nil)then
      keyy = myData.BonusRescue.key[ind][2]
      if(keyy == 0) then
        key[ind][2] = 0
      elseif(keyy == 'w') then
        key[ind][2] = 100
...
local function myscientist( )
    print( 'Being called' )
  sciencex = myData.BonusRescue.scientist[1]
  if(sciencex == 1) then
  ...
  sciencey = myData.BonusRescue.scientist[2]
  if(sciencey == 'w') then
...
function scene:show( event )
    ...
    if ( phase == "will" ) then
        ...
        --setting keys
        for i=1,3,1 do
            if(myData.BonusRescue.key[i][1]~=nil) then
                setkeys(i)
        ...
        --setting walls
        i = 1
        while(myData.BonusRescue.walls[i] ~= nil) do
            currwall = "wall"..(myData.BonusRescue.walls[i])
            currdata = myData[currwall]
